...
function TrafficChart({ logs = [] }) {
  ...
  // Convert latest logs into chart data
  const chartData = logs
    ...
    .map((log) => ({
      time: log.timestamp
        ...
        : "--:--:--",
      traffic: Number(log.bytes || 0) / 1000,
    }));

  const peakTraffic = chartData.length
    ? Math.max(...chartData.map((d) => d.traffic))
    : 0;
